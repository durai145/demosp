...
app.post('/jsonSchema/:sjson', function (req, res) {
	...
	if (req.params.sjson == "basicDet.bson") {
		...
		res.send(require("./idb/sjson/"+req.params.sjson));
	}
});
...
function signToken(res, secretkey, callback) {
	...
	var token = jwt.sign(payload, secretkey, {
		complete: true,
		maxAge: ms(sessionExpSec),
		expiresInMinutes: sessionExpSec / 60
	});
...
function USSField() {
	...
	var retObj = {
		'group': 'USS',
		/*it has been chabged by durai on 02-Feb-2010*/
		'name': 'name',
		'label': 'label',
		'task': 'NONE',
		'desc': '',
		'htmlType': 'text',
		/* newly introduced in USS05*/
